...
pub fn traverse_dir(path: PathBuf) -> Result<DirTree, Box<dyn Error>> {
...
fn traverse_recursive(tree: &mut DirTree, path: PathBuf, parent_idx: usize) -> Result<u64, Box<dyn Error>> {
    ...
    for item in reader {
        ...
        let metadata = symlink_metadata(&item_path)?;
        if metadata.is_file() {
